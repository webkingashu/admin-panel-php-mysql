...
function myRealPath($path) {
 ...
 function getRealPath($value)
 {
	...
	function getFileUrl($value)
	{
		...
		$appDir = preg_replace('#admin\/include\/tinymce\/ajaxfilemanager\/[^.]+\.php#', '', ltrim($_SERVER['PHP_SELF'], '/'));
		return $protocol . "://" .  addTrailingSlash(backslashToSlash($_SERVER['HTTP_HOST'])) . $appDir . removeBeginingSlash(backslashToSlash($output));
	}
